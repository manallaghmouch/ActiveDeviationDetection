                        ...
                        if nr_of_y_values > 1:
                            ...
                            while iteration <= number_of_AL_iterations:
                                ...
                                df = pd.DataFrame({
                                    ...
                                    'case:concept:name': test_set['case:concept:name']
                                })

                                df_sorted = df.sort_values('confidence')
                                df_least_confident = df_sorted.iloc[:number_of_cases_per_iteration]

                                # print('10 least confident predictions')
                                # print(df_least_confident)

                                # 3. ADD LEAST CONFIDENT CASES TO TRAINING SET
                                least_confident_cases = df_least_confident['case:concept:name'].tolist()
                                new_training_data = test_set[test_set['case:concept:name'].isin(least_confident_cases)]
                                ...
                                test_set = test_set[~test_set['case:concept:name'].isin(least_confident_cases)]
